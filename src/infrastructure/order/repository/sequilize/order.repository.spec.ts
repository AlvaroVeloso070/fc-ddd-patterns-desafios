...
describe("Order repository test", () => {
  ...
  it("should update an order", async () => {
    ...
    order1.items.push(orderItem2);
    await orderRepository.update(order1);

    const orderModelUopdated = await OrderModel.findOne({where: {id: order1.id}, include: ["items"]});

    expect(orderModelUopdated.toJSON()).toStrictEqual({
      id: "123",
      customer_id: "123",
      total: order1.total(),
      items: orderModelUopdated.items.map((item) => ({
        id: item.id,
        name: item.name,
        order_id: item.order_id,
        price: item.price,
